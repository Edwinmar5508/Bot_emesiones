...
CR_AUTH_URL      = "https://beta-api.crunchyroll.com/auth/v1/token"
CR_BROWSE_URL    = "https://beta-api.crunchyroll.com/content/v1/browse"
CR_EP_URL        = "https://beta-api.crunchyroll.com/cms/v2/US/M3/en-US/objects/{ep_id}"
...
async def check_and_notify(bot: Bot):
    ...
    if first_run:
        ...
        for ep in reversed(to_send):
            try:
                await send_episode(bot, ep)
                await asyncio.sleep(2)
            except Exception:
                pass

    else:
        new_eps = [ep for ep in episodes if ep["id"] not in seen]

        if not new_eps:
            log.info("🔍 Sin episodios nuevos.")
            return

        log.info(f"🆕 {len(new_eps)} episodios nuevos.")

        # Guardar ANTES de enviar para evitar reenvíos si hay crash
        for ep in new_eps:
            seen.add(ep["id"])
        save_seen(seen)
...
async def polling_loop(bot: Bot):
    log.info(f"🤖 Bot activo. Revisando cada {CHECK_INTERVAL // 60} min...")
    while True:
...
async def cmd_start(update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "🎌 <b>Crunchyroll Notifier Bot</b>\n\n"
        "<b>Comandos:</b>\n"
        "/login &lt;cookie_etp_rt&gt; — Autenticarse\n"
        "/check — Revisar episodios ahora\n"
        "/status — Estado del bot\n"
        "/reset — Borrar historial (reenvía los últimos 5)\n",
        parse_mode="HTML",
    )


async def cmd_login(update, context: ContextTypes.DEFAULT_TYPE):
...
async def cmd_check(update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text("🔍 Revisando episodios...")
...
async def cmd_status(update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if token_data:
        ...
        expires = f"✅ Expira en {remaining // 60} min"
    await update.message.reply_text(
        f"📊 <b>Estado del Bot</b>\n\n"
        f"🔑 Token CR: {expires}\n"
        f"📺 Episodios vistos: {len(seen)}\n"
        f"⏱ Intervalo: {CHECK_INTERVAL // 60} min\n"
...
async def cmd_reset(update, context: ContextTypes.DEFAULT_TYPE):
    if os.path.exists(SEEN_FILE):
        os.remove(SEEN_FILE)
    await update.message.reply_text(
        "🔄 Historial borrado. Usa /check para reenviar los últimos 5 episodios."
    )
